                                ...
                                <div v-if="hasDiscount(store.ctsp)">
                                    <div class="d-flex align-items-baseline gap-2 mb-1">
                                        ...
                                        <span class="badge bg-danger-subtle text-danger"
                                            v-if="discountPercent(store.ctsp)">-{{ discountPercent(store.ctsp)
                                            }}%</span>
                                    </div>
                                    <div class="small text-muted">Giá đã gồm VAT (nếu có)</div>
                                </div>
                                <div v-else>
                                    <div class="h3 text-danger mb-1">{{ formatVND(store.ctsp.giaBan ??
                                        store.ctsp.giaSauGiam) }}</div>
                                    <div class="small text-muted">Giá đã gồm VAT (nếu có)</div>
                                </div>
                            </div>
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <span class="text-end">
                                                <template v-if="hasDiscount(v)">
                                                    <span class="fw-semibold text-danger">{{ formatVND(v.giaSauGiam)
                                                        }}</span>
                                                    <small class="text-muted ms-1"><del>{{ formatVND(v.giaTruocGiam)
                                                            }}</del></small>
                                                </template>
                                                <template v-else>
                                                    <span class="fw-semibold text-danger">{{ formatVND(v.giaBan ??
                                                        v.giaSauGiam) }}</span>
                                                </template>
                                            </span>
                                        </div>
                    ...
                    <tbody>
                        <tr v-for="row in attrRows" :key="row.key">
                            ...
                            <td>
                                <span>{{ row.value }}</span>
                                <!-- swatch màu -->
                                <span v-if="row.hex" class="ms-2 d-inline-block align-middle" :style="{
                                    width: '16px',
                                    height: '16px',
                                    borderRadius: '4px',
 ...
 * - Lấy danh sách biến thể (CTSP) theo ID sản phẩm
 * - Chọn biến thể đầu tiên làm mặc định
 */
watch(
    () => route.params.id,
    async (sanPhamId) => {
        ...
        try {
            ...
            await store.fetchVariants(sanPhamId);

            // Nếu có biến thể, set biến thể đầu tiên làm chi tiết hiện tại
            if (store.variants && store.variants.length > 0) {
                store.ctsp = store.variants[0];
            }
        } catch (err) {
